...
def _ensure():
    with connect() as con:
        ...
        cols={r['name'] for r in con.execute('PRAGMA table_info(curricula)').fetchall()}
...
def _sample_material(workspace_id:int, document_ids:list[int], max_chars:int=42000)->str:
    rows=list(iter_chunks(workspace_id, document_ids))
    if not rows: raise ValueError('Nessun contenuto disponibile nel workspace.')
...
def create_curriculum(workspace_id:int, title:str, goal:str, document_ids:list[int], max_nodes:int=18)->int:
    _ensure(); material=_sample_material(workspace_id,document_ids)
...
MATERIALE:\n{material}'''
    ...
    with connect() as con:
        cur=con.execute('INSERT INTO curricula(workspace_id,title,goal,document_ids_json,created_at) VALUES(?,?,?,?,?)',(workspace_id,title,goal,json.dumps(document_ids),now)); cid=int(cur.lastrowid)
        known=[]
...
def list_curricula(workspace_id:int):
    _ensure()
    with connect() as con:
        return con.execute('SELECT * FROM curricula WHERE workspace_id=? ORDER BY id DESC',(workspace_id,)).fetchall()
...
def curriculum_document_ids(workspace_id:int, curriculum_id:int)->list[int]:
    _ensure()
    with connect() as con:
        r=con.execute('SELECT document_ids_json FROM curricula WHERE id=? AND workspace_id=?',(curriculum_id,workspace_id)).fetchone()
    return json.loads(r['document_ids_json']) if r else []


def next_node(workspace_id:int, curriculum_id:int):
    nodes=curriculum_nodes(curriculum_id); by_title={r['title']:r for r in nodes}
    candidates=[]
    for r in nodes:
        if r['status']=='done': continue
        prereq=json.loads(r['prerequisites_json'])
        ready=all((p in by_title and (by_title[p]['status']=='done' or mastery_for(workspace_id,p)>=.68)) for p in prereq)
        if ready:
            mastery=mastery_for(workspace_id,r['title']); priority=(1-mastery)*.7+float(r['importance'])*.3
...
def delete_curriculum(workspace_id:int, curriculum_id:int):
    _ensure()
    with connect() as con:
        row=con.execute('SELECT 1 FROM curricula WHERE id=? AND workspace_id=?',(curriculum_id,workspace_id)).fetchone()
        if not row: return
        con.execute('DELETE FROM curriculum_nodes WHERE curriculum_id=?',(curriculum_id,))
        con.execute('DELETE FROM curricula WHERE id=?',(curriculum_id,))
