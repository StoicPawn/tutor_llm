...
def new_workspace(payload:WorkspaceIn):
    try: return dict(get_workspace(create_workspace(payload.name,payload.description,payload.goal)))
    except Exception as exc: raise HTTPException(status_code=400,detail=str(exc))
@app.get('/workspaces/{workspace_id}/documents')
def documents(workspace_id:int): return [dict(r) for r in list_documents(workspace_id)]
...
def knowledge_graph(workspace_id:int): return graph(workspace_id)
@app.get('/workspaces/{workspace_id}/reviews/due')
def reviews_due(workspace_id:int,limit:int=20): return [dict(r) for r in due_reviews(workspace_id,limit)]
@app.get('/workspaces/{workspace_id}/reviews/upcoming')
def reviews_upcoming(workspace_id:int,limit:int=20): return [dict(r) for r in upcoming_reviews(workspace_id,limit)]
@app.post('/reviews')
def review(payload:ReviewIn): return record_review(payload.workspace_id,payload.concept,payload.score)
@app.post('/exercises/sessions')
def exercise_start(payload:ExerciseStartIn):
    try: return exercise_state(start_exercise_session(payload.workspace_id,payload.topic,payload.document_ids,payload.n,payload.epistemic_mode))
    except Exception as exc: raise HTTPException(status_code=400,detail=str(exc))
...
def ask(payload:TutorRequest):
    try:
        answer,sources=answer_question(payload.workspace_id,payload.topic,payload.document_ids,payload.epistemic_mode); return {'content':answer,'sources':sources}
    except Exception as exc: raise HTTPException(status_code=400,detail=str(exc))
@app.post('/tutor/lesson')
def lesson(payload:TutorRequest):
    try:
        content,sources=build_lesson(payload.workspace_id,payload.topic,payload.lesson_mode,payload.document_ids,payload.epistemic_mode); lesson_id=save_lesson(payload.workspace_id,payload.topic,payload.lesson_mode,content,sources,payload.epistemic_mode); return {'lesson_id':lesson_id,'content':content,'sources':sources}
    except Exception as exc: raise HTTPException(status_code=400,detail=str(exc))
@app.post('/tutor/summary')
def summary(payload:TutorRequest):
    content,sources=summarize(payload.workspace_id,payload.topic,payload.document_ids,payload.epistemic_mode); return {'content':content,'sources':sources}
@app.post('/tutor/deepen')
def deepen_topic(payload:TutorRequest):
    content,sources=deepen(payload.workspace_id,payload.topic,payload.document_ids,payload.epistemic_mode); return {'content':content,'sources':sources}
@app.post('/tutor/exercises')
def exercises(payload:TutorRequest):
    content,sources=build_exercises(payload.workspace_id,payload.topic,payload.document_ids,epistemic_mode=payload.epistemic_mode); return {'content':content,'sources':sources}
@app.post('/tutor/reasoning')
def reasoning(payload:TutorRequest):
    content,sources=build_reasoning(payload.workspace_id,payload.topic,payload.document_ids,payload.epistemic_mode); return {'content':content,'sources':sources}
...
@app.post('/sessions')
def new_session(payload:SessionIn): return dict(get_session(start_session(payload.workspace_id,payload.learning_goal)))
@app.patch('/sessions/{session_id}')
def patch_session(session_id:int,payload:SessionContextIn):
    update_session(session_id,payload.workspace_id,current_document_id=payload.current_document_id,current_page=payload.current_page,selected_text=payload.selected_text,current_concept=payload.current_concept,learning_goal=payload.learning_goal,state=payload.state); return dict(get_session(session_id))
@app.delete('/sessions/{session_id}')
def close_session(session_id:int,workspace_id:int): end_session(session_id,workspace_id); return {'ok':True}
